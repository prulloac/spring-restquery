    ...
    switch (condition.getOperation()) {
      ...
      case EQUALS_IGNORE_CASE:
        return criteriaBuilder.equal(
            criteriaBuilder.upper(root.get(condition.field)),
      ...
      case LIKE_OR_NULL:
        return criteriaBuilder.or(
            criteriaBuilder.like(root.get(condition.field), "%" + condition.value + "%"),
      ...
      case IN:
        CriteriaBuilder.In<String> in =
            criteriaBuilder.in(root.get(condition.field).as(String.class));
        Arrays.stream(condition.value.toString().split(",")).forEach(in::value);
        return in;
      case NOT_IN:
        CriteriaBuilder.In<Object> notIn =
            criteriaBuilder.in(root.get(condition.field).as(String.class));
        Arrays.stream(condition.value.toString().split(",")).forEach(notIn::value);
