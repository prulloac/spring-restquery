...
public interface SpecificationBuilder {
  ...
  private static SearchCondition createSearchCondition(String filter) {
    ...
    SearchCondition criteria =
        new SearchCondition().setField(split[0]).setOperation(QueryOperation.parse(split[1]));
  ...
  private static boolean isValidFilterSyntax(String[] filters) {
    String regex =
        "^\\w*:("
            + QueryOperation.UNARY_OPERATORS_REGEX
            + "|"
            + QueryOperation.BINARY_OPERATORS_REGEX
            + ":\\w*)$";
    return Arrays.stream(filters)
        .allMatch(filter -> Pattern.compile(regex).matcher(filter).matches());
  }

  private static List<String> filterFilters(String[] filters, List<String> fields) {
    Assert.isTrue(
        isValidFilterSyntax(filters),
  ...
  static <T> Specification<T> build(String[] filters, Class<T> entityClass) {
    ...
    List<DynamicSpecification<T>> cleansedFilters =
        filterFilters(filters, fields).stream()
    ...
    for (int i = 1; i < cleansedFilters.size(); i++) {
      result = Objects.requireNonNull(Specification.where(result)).and(cleansedFilters.get(i));
    }
    return result;
  }
}
